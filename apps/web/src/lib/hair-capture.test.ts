...
describe('hair capture quality', () => {
  it('requires the correct direction and rejects extreme turns', () => {
    expect(requiredPose('FRONT', 0.02)).toBe(true);
    expect(requiredPose('LEFT', -0.2)).toBe(true);
    expect(requiredPose('RIGHT', 0.2)).toBe(true);
    expect(requiredPose('LEFT', 0.2)).toBe(false);
    expect(requiredPose('RIGHT', -0.2)).toBe(false);
    expect(requiredPose('RIGHT', 0.8)).toBe(false);
  });
